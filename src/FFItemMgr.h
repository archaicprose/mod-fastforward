...
public:
	FFItemMgr(uint8 pLevel, uint8 pRace, uint8 pSpec) {
		specInfo = GetSpecInfo(pSpec);
		itemScorer = new FFItemScorer(specInfo->ffspec, pSpec);
		race = pRace;
		level = pLevel;
	}
	~FFItemMgr() {
